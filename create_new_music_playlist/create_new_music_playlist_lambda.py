...
# to add only featured tracks to the playlist
...
def get_new_music_playlist_id(sp, new_playlist_name, user_id):
    top_playlist_details = sp.current_user_playlists(limit=1)
# must keep the new music playlist at the top if you're going to run it again
...
def add_new_music_playlist_details(sp, user_id, playlist_uri, event):
    ...
    playlist_desc1 = "This is a new music playlist created from \
                      code by John Wilson (bonjohh on spotify). \
                      It was created "
    playlist_desc2 = "by taking the featured album \
                      or EP tracks (excluding singles tracks) from these " \
                      + str(playlist_number) + " playlists: "
    playlist_SPOTIFY_NM = "New Music Friday by Spotify"
    playlist_GETALTERNATIVE_NM = "The Alternative New Music \
                                  Friday by getalternative"
    ...
    while count < playlist_number:
        if 'SPOTIFY_NM' in event:
            if event['SPOTIFY_NM'] and \
                    playlist_SPOTIFY_NM not in playlist_desc:
        ...
        if 'GETALTERNATIVE_NM' in event:
            if event['GETALTERNATIVE_NM'] and \
                    playlist_GETALTERNATIVE_NM not in playlist_desc:
        ...
        if 'NPRMUSIC_NM' in event:
            if event['NPRMUSIC_NM'] and \
                    playlist_NPRMUSIC_NM not in playlist_desc:
...
def decide_what_playlists_to_pull_from(sp, event):
    spotify_nm = 'spotify:playlist:37i9dQZF1DX4JAvHpjipBk'
    getalternative_nm = 'spotify:playlist:6y4wz0Gmh2nMlBMjxduLCi'
    nprmusic_nm = 'spotify:playlist:5X8lN5fZSrLnXzFtDEUwb9'
    if 'SPOTIFY_NM' in event:
        if event['SPOTIFY_NM']:
            NEW_MUSIC_PLAYLISTS_LIST.append(spotify_nm)
    if 'GETALTERNATIVE_NM' in event:
        if event['GETALTERNATIVE_NM']:
            NEW_MUSIC_PLAYLISTS_LIST.append(getalternative_nm)
    if 'NPRMUSIC_NM' in event:
        if event['NPRMUSIC_NM']:
            NEW_MUSIC_PLAYLISTS_LIST.append(nprmusic_nm)
    if 'SPOTIFY_RR' in event:
        if event['SPOTIFY_RR']:
...
def main(event, context):
    ...
    add_new_music_playlist_details(sp, user_id, playlist_id, event)
# add new music playlist description
    ...
    for playlist_uri in NEW_MUSIC_PLAYLISTS_LIST:
        ...
        total_tracks_list = show_tracks(new_playlist_tracks, sp,
                                        new_playlist_name, total_tracks_list,
                                        playlist_id)
    ...
    create_status = str(total_tracks_set_length) + " songs added to \
