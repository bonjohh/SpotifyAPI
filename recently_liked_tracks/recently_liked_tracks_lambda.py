...
# must keep the new music playlist at
# the top if you're going to run it again
    playlist_id = ''
    for playlist in playlists['items']:
        if playlist['name'] == new_playlist_name:
            ...
            break
    if playlist_id == '':
        playlist_id = create_recently_liked_playlist(sp,
                                                     user_id,
                                                     new_playlist_name)
    return playlist_id
...
def add_new_music_playlist_details(sp, user_id, playlist_uri):
    ...
    playlist_desc1 = "This is a playlist created from code by John Wilson \
        (bonjohh on spotify). It was created "
    playlist_desc2 = "from songs liked in the past two weeks \
        (since " + str(playlist_date) + ")."
    playlist_desc = playlist_desc1 + playlist_desc2
    sp.user_playlist_change_details(user_id,
                                    playlist_id, description=playlist_desc)
...
def main(event, context):
    ...
    playlist_id = get_recently_liked_playlist_id(sp,
                                                 new_playlist_name, user_id)
    ...
    create_status = str(total_tracks_set_length) + " songs added to playlist: \
        [" + new_playlist_name + "]"
